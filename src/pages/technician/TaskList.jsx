  ...
  if (loading) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="mt-2 text-secondary">Loading assignments...</p>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {tasks.map((task) => (
          <div 
            key={task.id}
            className="group bg-card rounded-2xl border border-subtle overflow-hidden hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 flex flex-col"
          >
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <div className="mt-auto pt-5 border-t border-gray-50 flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-[10px] text-secondary uppercase tracking-widest font-bold">Deadline</span>
                  <span className={`text-sm font-semibold ${
                    new Date(task.deadline) < new Date() ? 'text-red-500' : 'text-primary'
                  }`}>
      ...
      {tasks.length === 0 && (
        <div className="bg-card rounded-2xl border-2 border-dashed border-subtle p-12 text-center mt-8">
          <div className="w-16 h-16 bg-surface rounded-full flex items-center justify-center mx-auto mb-4">
            <svg className="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
          </div>
          <h3 className="text-xl font-bold text-primary mb-1">No tasks assigned</h3>
