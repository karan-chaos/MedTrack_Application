...
public class SupplierPerformanceServiceTest {
        ...
        @Test
        void getPerformance_DeliveredAfterETA_CountedAsLate() {
                Long supplierId = 50L;
                LocalDateTime eta = LocalDateTime.now().minusHours(5);
                LocalDateTime actualDelivery = LocalDateTime.now().minusHours(1); // after ETA

                // delayDetected is false but actualDeliveryDate > estimatedDeliveryDate
                ShipmentTracking lateByDate = ShipmentTracking.builder()
                                .id(1L).supplierId(supplierId)
                                .shipmentStatus(ShipmentStatus.DELIVERED)
                                .delayDetected(false) // scheduler hasn't run yet
                                .estimatedDeliveryDate(eta)
                                .actualDeliveryDate(actualDelivery)
                                .build();

                when(shipmentTrackingRepository.countBySupplierId(supplierId)).thenReturn(1L);
                when(shipmentTrackingRepository.countBySupplierIdAndDelayDetectedTrue(supplierId)).thenReturn(0L);
                when(shipmentTrackingRepository.findBySupplierIdAndShipmentStatus(supplierId, ShipmentStatus.DELIVERED))
                                .thenReturn(List.of(lateByDate));

                SupplierPerformanceResponse response = service.getPerformance(supplierId);

                // Should count as 0 on-time because actualDelivery > eta
                assertEquals(0L, response.getOnTimeShipments());
                assertEquals(0.0, response.getOnTimeDeliveryRate(), 0.001);
        }

        @Test
        void getPerformance_DeliveredBeforeETA_CountedAsOnTime() {
                Long supplierId = 60L;
                LocalDateTime eta = LocalDateTime.now().plusDays(2);
                LocalDateTime actualDelivery = LocalDateTime.now().minusHours(1); // before ETA

                ShipmentTracking earlyDelivered = ShipmentTracking.builder()
                                .id(1L).supplierId(supplierId)
                                .shipmentStatus(ShipmentStatus.DELIVERED)
                                .delayDetected(false)
                                .estimatedDeliveryDate(eta)
                ...
                when(shipmentTrackingRepository.countBySupplierIdAndDelayDetectedTrue(supplierId)).thenReturn(0L);
                when(shipmentTrackingRepository.findBySupplierIdAndShipmentStatus(supplierId, ShipmentStatus.DELIVERED))
                                .thenReturn(List.of(earlyDelivered));
